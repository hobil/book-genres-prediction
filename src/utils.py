...
def pickle_partial(obj, filename, parts=1):
    """
    Split a list or numpy array into given number of parts
    and pickle each of them separately.
    ...
    Keyword Arguments:
        parts {int} -- [description] (default: {1})
    """
    if parts == 1:
        ...
        for part in range(parts):
            ...
            pickle.dump(obj[part * part_size: (part + 1) *
                            part_size], open(filename_part, 'wb'))
...
def load_snippets(doc_size=200, word_tokenized=False, return_doc_ids=False,
                  new_split=False):
    if new_split:
        genres = pickle.load(open('../res/genres_snippets_ex_1.pkl', 'rb'))
        ids_train = pickle.load(open('../res/ids_train_1_v2_genre.pkl', 'rb'))
        ids_test = pickle.load(open('../res/ids_test_1_v2_genre.pkl', 'rb'))
    else:
    ...
    if word_tokenized:
        texts = unpickle(
            '../res/documents_{}_word_tokenize_tokens.pkl'.format(doc_size))
    else:
        texts = unpickle(
            '../res/documents_{}_simple_tokens.pkl'.format(doc_size))
    texts = np.array(texts)

    if doc_size == 3200:
        ids = np.array(genres_ids)
    else:
        ids = np.array(pickle.load(open('../res/documents_ids.pkl', 'rb')))
    ...
    if not return_doc_ids:
        return train_texts, test_texts, y_train, y_test
    else:
        ids_train = np.array(ids_train)
        ids_test = np.array(ids_test)
        return train_texts, test_texts, y_train, y_test, ids_train, ids_test


def load_glove_embedding(size=50, glove_file_path='../res/glove.6B.{}d.txt',
                         output_pickle_file='../res/glove_embeddings_{}.pkl'):
    output_file = output_pickle_file.format(size)
    if os.path.exists(output_file):
        return pickle.load(open(output_file, 'rb'))
    else:
        glove_vectors_file = glove_file_path.format(size)
        embeddings_index = {}
...
def load_catalog():
    ...
    download the catalog from the given URL.
    Returns
    -------
    [pandas.DataFrame]
        [description]
    ...
    df = pd.DataFrame.from_dict(
        metadata_catalog_dict, orient='index').set_index('id')
    return df
...
def tokenize_folder(input_folder, output_folder, tokenizer=gensim.utils.simple_preprocess):
    ...
    for filename in os.listdir(input_folder):
        i += 1
        logging.info(f'\r{i} out of {total_books} books processed - {filename}')
        try:
            with open(os.path.join(input_folder, filename), encoding="utf-8") as f:
                ...
                pickle.dump(tokens, open(
                    os.path.join(output_folder, doc_id), 'wb'))
        except Exception as e:
            logging.warning(e)


def load_corpus(folder, skip_first_n_words=None, words_in_document=None,
                samples_per_document=1, document_sampling_random=True,
                random_seed=42):
        ...
        for sample_id in range(samples_per_document):
            ...
            if document_sampling_random:
                if skip_first_n_words > (n_tokens - skip_first_n_words - w):
                    continue
                start_token_id = random.randint(
                    skip_first_n_words, n_tokens - skip_first_n_words - w)
            else:
